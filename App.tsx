...
const NotificationBanner: React.FC<{ message: string; onDismiss: () => void }> = ({ message, onDismiss }) => {
    ...
    <div className="fixed top-16 left-1/2 -translate-x-1/2 w-full max-w-md z-50 p-4">
        <div className="bg-white dark:glass-panel border-l-4 border-yellow-500 text-neutral-800 dark:text-neutral-100 p-4 rounded-r-lg shadow-lg flex items-start gap-3 animate-zoomIn">
            <AlertTriangleIcon className="w-5 h-5 flex-shrink-0 mt-0.5 text-yellow-500" />
            <div className="flex-1">
                <p className="font-bold text-yellow-500">{T.attention}</p>
                <p className="text-sm">{message}</p>
            </div>
            <button onClick={onDismiss} className="p-1 rounded-full hover:bg-neutral-200 dark:hover:bg-white/10">
...
const ServerSelectionModal: React.FC<ServerSelectionModalProps> = ({ isOpen, servers, serverUsage, onSelect, onClose }) => {
    ...
    const getUsageColor = (count: number) => {
        if (count < 10) return 'text-green-600 bg-green-100 dark:text-green-400 dark:bg-green-900/30';
        if (count < 20) return 'text-yellow-600 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30';
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-zoomIn" onClick={onClose}>
            <div className="w-full max-w-lg bg-white dark:glass-panel rounded-2xl shadow-2xl p-6 relative flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-white/10 text-neutral-500 dark:text-neutral-400 transition-colors"
                >
                ...
                <div className="text-center mb-6">
                 ...
                 <button
                    ...
                    className="w-full flex items-center justify-center gap-3 p-4 mb-6 bg-primary-600 hover:bg-primary-700 dark:bg-neon-gradient text-white rounded-xl shadow-lg dark:shadow-primary-500/20 hover:scale-[1.01] transition-all group"
                >
                ...
                <div className="border-b border-neutral-200 dark:border-white/10 mb-4 shrink-0"></div>
                    ...
                    {servers.map(serverUrl => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all group text-left ${isSelected ? 'bg-primary-50 border-primary-200 dark:bg-primary-900/20 dark:border-primary-500/50 ring-1 ring-primary-500/50' : 'bg-neutral-50 border-neutral-200 hover:bg-neutral-100 dark:bg-white/5 dark:border-white/5 dark:hover:border-primary-500/30 dark:hover:bg-white/10'}`}
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-lg ${isSelected ? 'bg-primary-100 text-primary-600 dark:bg-primary-500/20 dark:text-primary-400' : 'bg-white text-neutral-500 dark:bg-white/5 dark:text-neutral-400'}`}>
                                        <ServerIcon className="w-5 h-5" />
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <p className="font-bold text-sm capitalize text-neutral-800 dark:text-white">{T.server} {serverName.toUpperCase()}</p>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    {isSelected && <CheckCircleIcon className="w-5 h-5 text-primary-600 dark:text-primary-400" />}
                                    {!isSelected && <ChevronRightIcon className="w-5 h-5 text-neutral-400 group-hover:text-neutral-600 dark:group-hover:text-neutral-300" />}
                                </div>
                            </button>
                        );
                    })}
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadSettings = async () => {
        // Load theme from storage, default to dark
  ...
  if (!sessionChecked || (currentUser && isApiKeyLoading)) {
      return (
          <div className="flex items-center justify-center min-h-screen bg-neutral-100 dark:bg-midnight text-neutral-900 dark:text-white">
              <Spinner />
          </div>
      );
  }
  ...
  const PageContent = isBlocked ? (
    <div className="flex items-center justify-center h-full p-4">
      <div className="text-center p-8 sm:p-12 max-w-lg bg-white dark:glass-panel rounded-xl shadow-lg border border-neutral-200 dark:border-white/5">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-red-100 dark:bg-primary-900/50">
          <XIcon className="h-6 w-6 text-red-600 dark:text-primary-400" />
        </div>
        <h2 className="mt-5 text-xl font-bold text-neutral-900 dark:text-white sm:text-2xl">{blockMessage.title}</h2>
  ...
  return (
    <div className="flex h-screen bg-neutral-50 dark:bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] dark:from-slate-900 dark:via-[#0B1120] dark:to-black text-neutral-900 dark:text-neutral-100 font-sans overflow-hidden transition-colors duration-300">
      <Sidebar 
      ...
      <main className="flex-1 flex flex-col overflow-y-auto min-w-0 relative">
        {/* Sticky Header Wrapper - GLASS EFFECT */}
        <div className="sticky top-0 z-30 w-full bg-white dark:bg-slate-900/90 dark:backdrop-blur-md border-b border-neutral-200 dark:border-white/10 shadow-sm transition-colors duration-300">
            <header className="flex items-center justify-between p-2">
              <div className="flex items-center gap-2">
                <button onClick={() => setIsSidebarOpen(true)} className="p-2 lg:hidden text-neutral-600 dark:text-white" aria-label={T.openMenu}>
                  <MenuIcon className="w-6 h-6" />
                </button>
                 <LogoIcon className="w-28 text-neutral-800 dark:text-white" />
              </div>
              <div className="flex items-center gap-2 pr-2">
                  <ThemeSwitcher theme={theme} setTheme={setTheme} />
                   <button
                      onClick={() => setIsLogSidebarOpen(!isLogSidebarOpen)}
                      className={`p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-white/10 transition-colors ${isLogSidebarOpen ? 'bg-neutral-100 dark:bg-white/10' : ''}`}
                      aria-label={isLogSidebarOpen ? 'Close Console' : T.openConsole}
                      title={isLogSidebarOpen ? 'Close Console' : T.openConsole}
                  >
                      <TerminalIcon className={`w-5 h-5 ${isLogSidebarOpen ? 'text-primary-600 dark:text-primary-400' : 'text-neutral-500 dark:text-neutral-400'}`} />
                  </button>
                  <button
                  ...
                  >
                      <RefreshCwIcon className="w-5 h-5 text-neutral-500 dark:text-neutral-400" />
                  </button>
                  <ApiKeyStatus 
            ...
            {/* Announcement Ticker - High Contrast */}
            {activeView === 'home' && announcements.length > 0 && (
                <div className="w-full bg-black text-white px-4 py-2 text-sm font-medium overflow-hidden relative whitespace-nowrap border-b border-neutral-800 shadow-sm flex-shrink-0 max-w-[100vw]">
