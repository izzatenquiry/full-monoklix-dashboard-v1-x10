...
const ProductReviewView: React.FC<ProductReviewViewProps> = ({ onReEdit, onCreateVideo, currentUser, onUserUpdate, language }) => {
  ...
  const generateSceneImage = async (index: number, attempt = 1, excludedServers: string[] = [], initialServer?: string) => {
    ...
    const tokenMode = sessionStorage.getItem('monoklix_token_mode') || 'personal_only';
    ...
    if (isPersonalOnly) {
        ...
        // Use specific server for batch distribution
    ...
    try {
        ...
        if (!isPersonalOnly && attempt < 3 && !isNonRetriable) {
            // Hybrid retry logic
            await generateSceneImage(index, attempt + 1, [...excludedServers, serverToUseUrl]);
        } else {
            // Final failure
            const userFriendlyMessage = handleApiError(e);
            setImageGenerationErrors(prev => {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-2xl shadow-sm dark:shadow-lg border border-neutral-200 dark:border-white/5 h-full overflow-y-auto custom-scrollbar space-y-8">
      <div>
      ...
      <div className={`transition-opacity duration-500 ${step2Disabled ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
      ...
      <div className={`transition-opacity duration-500 ${step3Disabled ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
