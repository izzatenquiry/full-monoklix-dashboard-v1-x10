...
const ApiIntegrationsPanel: React.FC<ApiIntegrationsPanelProps> = ({ currentUser, onUserUpdate, language, veoTokenRefreshedAt, assignTokenProcess }) => {
    ...
    const handleClaimSharedToken = useCallback(async (token: string) => {
        ...
        try {
            ...
            setTokenStatusMessage({ type: 'error', message: "An unexpected error occurred." });
        } finally {
                                ...
                                <div className="flex items-center gap-3 self-end sm:self-auto">
                                    ...
                                    <div className="flex items-center bg-neutral-100 dark:bg-neutral-800 rounded-lg p-1 border border-neutral-200 dark:border-neutral-700 h-8">
                                        ...
                                        <button 
                                            ...
                                            className={`px-3 h-full text-[10px] font-bold rounded-md transition-colors flex items-center justify-center gap-1.5 ${
                                                isCurrentToken 
                                                    ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 cursor-default' 
                                                    : 'text-primary-600 dark:text-primary-400 hover:bg-white dark:hover:bg-neutral-700 disabled:opacity-50 disabled:text-neutral-400'
                                            }`}
                                        >
                                            {isBeingClaimed ? <Spinner /> : isCurrentToken ? <><CheckCircleIcon className="w-3 h-3"/> Active</> : T.claim}
                                        </button>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow-sm space-y-8 h-fit">
                    ...
                    <div>
                        ...
                        <div className="mt-4 p-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg flex justify-between items-center">
                            ...
                            {currentUser.role === 'admin' || currentUser.status === 'lifetime' || currentUser.status === 'subscription' ? (
