...
import {
  ImageIcon, VideoIcon, SettingsIcon, BookOpenIcon, LogoutIcon, GalleryIcon, LogoIcon, XIcon, LibraryIcon, FileTextIcon, GraduationCapIcon, TrendingUpIcon, RobotIcon, MegaphoneIcon, DatabaseIcon, TelegramIcon
} from './Icons';
...
const getNavItems = (language: Language): NavItem[] => {
    ...
    return [
        ...
        { id: 'support-group', label: T.supportGroup, section: 'bottom', icon: TelegramIcon, isExternal: true, url: 'https://t.me/monoklix_support' },
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, onLogout, currentUser, isOpen, onClose, language }) => {
  ...
  return (
    <>
      ...
      ></div>
      <nav 
        className={`w-80 bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 dark:glass-panel p-5 flex flex-col transition-transform duration-300 ease-custom-ease z-40
      ...
      >
        <div className="mb-8 flex items-center justify-between">
          ...
          <button onClick={onClose} className="lg:hidden p-2" aria-label={T.closeMenu}>
            <XIcon className="w-6 h-6 text-neutral-600 dark:text-white" />
          </button>
        </div>
